...
.PHONY: git-hook-install git-hook-git-hook-dir-defined git-submodule-update git-submodule-reset
...
git-hook-install:
	...
	for hook in $(notdir $(wildcard ./$(GIT_HOOK_DIR)/*)); do \
		if [ -d "./$(GIT_HOOK_DIR)/$$hook" ]; then \
			echo "cannot install git hook for $$hook: ./$(GIT_HOOK_DIR)/$$hook is a directory"; \
			exit 1; \
		fi; \
		\
		if [ ! -x "./$(GIT_HOOK_DIR)/$$hook" ]; then \
			echo "cannot install git hook for $$hook: ./$(GIT_HOOK_DIR)/$$hook is not executable"; \
			exit 1; \
		fi; \
		\
		if [ -d ".git/hooks/$$hook" ]; then \
			echo "cannot install git hook for $$hook: .git/hooks/$$hook is a directory"; \
			exit 1; \
		fi; \
		\
